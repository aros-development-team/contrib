...
#ifdef __AROS__
/*
 * AROS socket functions are provided as macros via proto/socket.h.
 * Include it here so all iperf source files get socket function definitions.
 * proto/miami.h provides resolver functions (getaddrinfo, inet_ntop, etc.).
 */
#include <proto/socket.h>
#include <proto/miami.h>

/*
 * Tell other iperf headers that getline/daemon are handled here,
 * preventing prototype declarations that would conflict with our macros.
 */
#define HAVE_GETLINE 1

/* strsignal is not available on AROS */
#include <string.h>
#include <stdio.h>
#ifndef strsignal
static inline char *iperf_strsignal(int sig) {
    static char buf[24];
...
#include <stdio.h>
#include <stdlib.h>
static inline ssize_t iperf_getline(char **lineptr, size_t *n, FILE *stream) {
    size_t pos = 0;
    int c;
    if (*lineptr == NULL || *n == 0) {
        *n = 128;
...
#define getline(lineptr, n, stream) iperf_getline((lineptr), (n), (stream))

/*
 * AROS does not have struct rusage or getrusage().
 * Provide minimal stubs so cpu_util() compiles (returns 0).
 */
#include <sys/time.h>
#ifndef RUSAGE_SELF
...
/*
 * AROS does not have fork()/setsid().  Provide a no-op daemon() stub
 * (HAVE_DAEMON is set in iperf_config.h to suppress the built-in one).
 */
#include <unistd.h>
#ifndef HAVE_DAEMON
#define HAVE_DAEMON 1
#endif
static inline int iperf_daemon(int nochdir, int noclose) {
    (void)nochdir; (void)noclose;
    return 0; /* no-op on AROS */
}
#define daemon(a, b) iperf_daemon((a), (b))
